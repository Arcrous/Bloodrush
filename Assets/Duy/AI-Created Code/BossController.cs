...
namespace Unity.FPS.AI
{
    ...
    public class BossController : EnemyController
    {
        ...
        [Header("Melee Dash Parameters")]
        [Tooltip("Force applied when dashing forward for melee attack")]
        public float MeleeDashForce = 15f;

        [Tooltip("Duration of the melee dash")]
        public float MeleeDashDuration = 0.3f;
        ...
        IEnumerator PerformAttack()
        {
            ...
            switch (m_CurrentAttackType)
            {
                case BossAttackType.Melee:
                    ...
                    float meleeTimeout = 10f; // 10 seconds timeout

                    float distanceToTargetMelee = Vector3.Distance(transform.position, KnownDetectedTarget.transform.position);
                    if (distanceToTargetMelee > MeleeAttackRange)
                    {
                        // Play dash animation
                        if (Animator != null)
                        {
                        ...
                        // Dash forward
                        Debug.Log("Starting melee dash");
                        yield return StartCoroutine(DashCoroutine(transform.forward, MeleeDashForce, MeleeDashDuration));
                    }
                ...
                case BossAttackType.Ranged:
                    ...
                    if (DetectionModule.IsTargetInAttackRange)
                    {
                        ...
                        if (distanceToTarget < MinRangedAttackDistance)
                        {
                            ...
                            // Apply dash force
                            yield return StartCoroutine(DashCoroutine(dashDirection, DashForce, DashDuration));
                        }
        ...
        private IEnumerator DashCoroutine(Vector3 direction, float force, float duration)
        {
            Debug.Log($"Starting dash with force {force}, duration {duration}");
            float startTime = Time.time;
            NavMeshAgent.enabled = false;
            m_Rigidbody.isKinematic = true;

            Vector3 startPosition = transform.position;
            float targetDashDistance = force;
