...
public class EnemySpawner : MonoBehaviour
{
    ...
    [Tooltip("Delay before resetting the spawner after spawning")]
    public float spawnResetDelay = 20f;
    ...
    public void TriggerSpawn()
    {
        ...
        Invoke("ResetSpawner", spawnResetDelay); // Reset spawner after a delay
    }

    void ResetSpawner()
    {
        hasSpawned = false; // Reset the spawn state
        if (spawnParticlePrefab != null)
        {
            spawnParticlePrefab.Stop(); // Stop the particle system if it exists
        }
    }
