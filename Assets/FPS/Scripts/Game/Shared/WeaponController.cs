...
namespace Unity.FPS.Game
{
    ...
    {
        [Header("Sniper Only Setting")]
        [Tooltip("Exists solely to control the sniper's variable")]
        [SerializeField] public bool IsSniper = false;
        [SerializeField] int shotsFired = 0;
        [SerializeField] public UnityAction ultFinished;
        ...
        bool TryReleaseCharge()
        {
            if (IsCharging)
            {
                ...
                {
                    shotsFired++;
                    Debug.Log("Sniper shot fired: " + shotsFired);
                    if (shotsFired >= 5)
                    {
                        shotsFired = 0;
                        ultFinished?.Invoke();
